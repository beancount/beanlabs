...
def adjust_entry(
    ...
    return entry._replace(postings=new_postings, tags=new_tags)
...
def main():
    ...
    parser.add_argument("filename", action="store", help="Beancount filename to read")
    parser.add_argument("tag", action="store", help="Tag to select transactions")
    parser.add_argument(
        "inflows_account",
        action="store",
    ...
    parser.add_argument(
        "match_account", action="store", help="Explicit account to match against"
    )
    parser.add_argument(
        "-t",
        "--translate",
        action="append",
        help=(
            "Explicit account translations. This can be used to make "
    ...
    for entry in data.filter_txns(entries):
        if tag in entry.tags or any(
            posting.account == args.match_account for posting in entry.postings
        ):
            entry = adjust_entry(
                entry, args.match_account, args.inflows_account, args.tag, translate_map
            )
